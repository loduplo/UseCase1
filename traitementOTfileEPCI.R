...
write.csv2(pacaEpci, file="F:/Flexgrid/Sprint2/20180419pacaEpci.csv", row.names=FALSE,fileEncoding = "UTF-8",quote=FALSE)
#
...
#epciref<-read.csv2("F:/Flexgrid/Sprint1/R/REF/referentielEpci.csv",encoding = "UTF-8")
epciref<-read.csv2("F:/Flexgrid/Sprint2/clusterReferentielEpci.csv",encoding = "UTF-8")
# table(epciref$nature)
# epciref$nature <- as.factor(epciref$nature)
# str(epciref$nature)
#CA, CC, Métropole
#levels(epciref$nature)<-c("CA", "CC","Métropole")
...
fileEpci <-read.csv2(file="F:/Flexgrid/Sprint2/20180419pacaEpci.csv", fileEncoding = "UTF-8")
...
levels(clusterEpci$kmeancol) <- c("green","yellow2","orange2","blue","red","red4")
